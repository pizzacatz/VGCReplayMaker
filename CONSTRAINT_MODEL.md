...
- **Multi-hit moves:** the HP delta is a sum of sub-hits. Unless each sub-hit's number is read separately, the event is inherently `composite` (schema §6) and the solver ignores it. If sub-hits are separable, each is its own clean factor.
- **Variable-power moves** (power depends on speed, weight, HP, etc.): the factor is still well-defined *if* the power's dependency is computable from known/candidate values, but it adds coupling (e.g. a speed-based move ties the damage factor to Spe too). Default: support the common cases explicitly; for any move whose power can't be pinned from available data, mark its hits `unresolved` rather than guess the power.
